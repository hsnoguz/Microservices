...
namespace FreeCourse.Services.Catalog.Services
{
    internal class CategoryService : ICategoryService
    {
        ...
        public async Task<Response<CategoryDto>> GetResponseAsync(string id)
        {
        ...
        public async Task<Response<CategoryDto>> GeByIdAsync(string id)
        {
            var category = await _categoryCollection.Find<Category>(x => x.Id == id).FirstOrDefaultAsync();
            if (category == null)
            {
            ...
            return Response<CategoryDto>.Succes(categoryDto, 200);
        }
    }
}
